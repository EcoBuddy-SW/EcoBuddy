...
const AttendanceScreen = () => {
    ...
    useEffect(() => {
        const loadAttendance = async () => {
            try {
                const result = await axios.post(`http://${context.ip}:3003/attendance`, { userId: userId, date: today });
                const attendanceData = result.data;
    
                if (attendanceData.find((data) => data.date === today)) {
                    setAttendance(true);
                }
            } catch (error) {
    ...
    const handleAttendance = async () => {
        if (attendance) {
            ...
            try {
                const result = await axios.post(`http://${context.ip}:3003/attendance/update`, {
                    userId: userId,
                    date: today,
                    points: 10000,
                });
        
                if (result.data.message === "이미 출석체크를 했습니다.") {
                    setModalMessage('출석이 이미 완료되었습니다.');
                } else {
                    setAttendance(true);
                    ...
                    setModalMessage('출석이 완료되었습니다. 마이페이지 → 포인트 상점에서 포인트 점수를 확인하세요.');
                }
            } catch (error) {
