...
class HomeScreen extends Component {
  ...
  async getLocation() {
    ...
    let { status } = await Location.requestForegroundPermissionsAsync({accuracy: Location.Accuracy.Highest, maximumAge: 10000});
    console.log(status);
    if (status !== 'granted') {
   ...
   let newLocation = await Location.getCurrentPositionAsync({accuracy: Location.Accuracy.Highest, maximumAge: 10000});
    console.log('입니다.');
