...
export default function SearchResultScreen() {
    ...
    const goToMap = (location) => {
        navigation.navigate('지도', { location }); // 'Map' 스크린으로 이동
    };

    const goToDetail = (info) => {
        navigation.navigate('세부사항',{url : info}); 
    };
      ...
      <View>
        ...
        <View style={styles.resultContainer}>
          ...
          {separationResults.map((result,index) => (
            <View key={index} style={[styles.subContainer, { marginBottom: 15 }]}>
              ...
              <Text style={[styles.text, { marginStart: 15, marginBottom: 30 }]}>{result.location}</Text>
              {/* 버튼 */}
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                ...
                {/* 정확한 위치 버튼 */}
                <TouchableOpacity onPress={() => goToMap(result.location)} style={[styles.btn, { backgroundColor: '#FAEDFF', marginStart: 10 }]}>
        ...
        <View style={[styles.resultContainer, { marginTop: 30 }]}>
          ...
          {/* 서브컨테이너 */}
          {welfareResults.map((result,index) => (
            <View key={index} style={[styles.subContainer, { marginBottom: 15 }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                <Text style={[styles.text, { fontSize: 15 }]}>{result.name}</Text>
                <View style={{ flex: 1, alignItems: 'flex-end' }}>
